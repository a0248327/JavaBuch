	...
	<c:forEach var="category" items="${ categoryList }">
		<div class="t">
			<table cellspacing="0" cellpadding="0" width="100%">
				<tr>
					<th class="h" colspan="6"><a class="closeicon fr"
						style="cursor: pointer;" onclick="return IndexDeploy('85',0)"><img
							id="img_85" src="images/yellow/index/cate_fold.gif" /> </a>
						<h2>
							&raquo; <a
				...
				<tr class="tr2">
					...
					<td class="tal y-style e" style="width: 5%">主题</td>
					<td class="tal y-style e" style="width: 5%">文章</td>
					<td class="tal y-style f" style="width: 40%">最后发表</td>
						...
						<tr class="tr3 f_one">
							<td class="icon tac" width="45"><a
								href="<html:rewrite action="/board?action=list" />&board.id=${ board.id }"><img
									src="images/yellow/new.gif" /> </a></td>
							<th>
								<h3 class="b">
									<a
										href="<html:rewrite action="/board?action=list" />&board.id=${ board.id }">${
										board.name }</a>
								</h3> <br /> <span class="smalltxt gray">${ board.description }</span>
							</th>
							...
							<th><c:if test="${ board.lastThread != null }">
									...
									<span class="f12"><a
										href="<html:rewrite action="/person?action=view" />&person.id=${ board.lastThread.author.id }">${
											board.lastThread.author.account }</a>
									</span>
									...
									<span class="f12"><a
										href="<html:rewrite action="/person?action=view" />&person.id=${ board.lastReply.author.id }">${
											board.lastReply.author.account }</a>
									</span>
